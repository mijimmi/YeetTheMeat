...
if (is_processing) {
    ...
    if (progress_pct > 0.02) {
        ...
        // Draw fill with gradient strokes
        for (var fx = fill_left; fx < fill_right; fx += 3) {
            var fill_progress = (fx - fill_left) / (bar_width - 8);
            
            // Green to yellow color
            var fill_color = merge_color(make_color_rgb(100, 190, 80), make_color_rgb(230, 180, 50), fill_progress);
    ...
    for (var i = 0; i < 4; i++) {
        ...
        draw_sprite_ext(cloud_spr, 0, cloud_x, cloud_y, cloud_scale, cloud_scale, cloud_rot, c_white, cloud_alpha);
    }

    // Popcorn particles — occasional bursts that arc away
    for (var i = 0; i < ds_list_size(popcorn_particles); i++) {
        var _p = popcorn_particles[| i];
        draw_sprite_ext(_p[6], 0, _p[0], _p[1], _p[7], _p[7], 0, c_white, _p[5]);
    }
...
if (p1 != noone && global.p1_closest_station == id && !is_processing) {
    ...
    if (dist <= interact_range) {
        ...
        // Station has sliced food, player empty-handed
